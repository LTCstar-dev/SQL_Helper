...
db_config = {
    ...
    'password': 'root',
    'ai_api_key': '',
    'ai_api_url': 'https://api.openai.com/v1/chat/completions',  # OpenAI API端点
...
def call_ai_api(user_input, table_structure):
    ...
    try:
        ...
        data = {
            "model": db_config['selected_model'],
...
def generate_sql_with_ai():
    ...
    try:
        ...
        if response and 'choices' in response:
            current_ai_sql = response['choices'][0]['message']['content']
            explanation = "根据您的需求生成了以下SQL语句。"
...
def execute_ai_sql():
    ...
    # 提取纯SQL语句
    sql_to_execute = current_ai_sql.split('\n', 1)[0].strip()
...
def open_settings():
    ...
    settings_window.geometry("400x550")  # 增大窗口高度
    ...
    api_key_entry.insert(0, db_config['ai_api_key'])
    ...
    api_url_entry.insert(0, db_config['ai_api_url'])

    model_var = tk.StringVar(value=db_config['selected_model'])
    model_label = tk.Label(settings_window, text="模型选择:")
    model_label.grid(row=8, column=0, padx=10, pady=5, sticky='e')
    model_combobox = ttk.Combobox(settings_window, textvariable=model_var)
    model_combobox['values'] = (
        "gpt-4.1", "gpt-4.1-mini", "gpt-4.1-nano",
        "gpt-4o", "gpt-4.5", "gpt-4o-mini",
        "o3", "o3-mini", "o4-mini"
    )
    model_combobox.grid(row=8, column=1, padx=10, pady=5)
    ...
    def save_settings():
        ...
        db_config['selected_model'] = model_var.get()
...
def show_help():
    help_text = """
    使用说明:
    1. 在设置中配置OpenAI API Key和URL。
    2. 选择要使用的模型（例如：gpt-4o-mini）。
    3. 选择一个表后，在AI助手区域输入你的SQL需求。
    4. 点击"生成SQL"按钮，AI将根据你的需求生成SQL语句。
    5. 审核生成的SQL后，点击"执行SQL"按钮执行。
    """
