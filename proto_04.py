...
def generate_chart():
    ...
    # 选择图表类型
    chart_type_label = tk.Label(chart_window, text="选择图表类型:")
    chart_type_label.grid(row=0, column=0, padx=10, pady=10)
    chart_type_var = tk.StringVar(value="bar")
    chart_type_menu = ttk.Combobox(chart_window, textvariable=chart_type_var, values=["bar", "line", "pie"])
    chart_type_menu.grid(row=0, column=1, padx=10, pady=10)

    # 选择 X 轴
    x_axis_label = tk.Label(chart_window, text="选择 X 轴:")
    x_axis_label.grid(row=1, column=0, padx=10, pady=10)
    ...
    # 选择 Y 轴
    y_axis_label = tk.Label(chart_window, text="选择 Y 轴:")
    y_axis_label.grid(row=2, column=0, padx=10, pady=10)
    ...
    def plot_chart():
        ...
        try:
            ...
            if chart_type == "bar":
                ...
                # 检查数据类型
                print(df.dtypes)  # 确保 X 和 Y 轴是数值类型
                print(df.head())  # 预览数据

                if df[x_axis].isnull().any() or df[y_axis].isnull().any():
                ...
                if df[x_axis].isnull().any() or df[y_axis].isnull().any():
                    messagebox.showerror("错误", "所选列包含无效数据，请检查数据格式")
                    return
                df.plot(kind="pie", y=y_axis, labels=df[x_axis], ax=ax)
            ...
            chart_canvas.draw()
            chart_canvas.get_tk_widget().grid(row=4, column=0, columnspan=2, padx=10, pady=10)
        except Exception as e:
            messagebox.showerror("错误", f"生成图表失败: {e}")

    plot_button = tk.Button(chart_window, text="生成图表", command=plot_chart)
