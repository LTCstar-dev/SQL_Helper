...
def generate_chart():
    ...
    chart_window.geometry("700x900")  # 增加窗口高度以容纳更多选项
    ...
    # **新功能：图表设置选项**
    show_legend_var = tk.BooleanVar(value=True)  # 默认显示图例
    show_legend_check = tk.Checkbutton(chart_window, text="显示图例", variable=show_legend_var)
    show_legend_check.grid(row=5, column=0, columnspan=2, pady=5)

    show_grid_var = tk.BooleanVar(value=True)  # 默认显示网格
    show_grid_check = tk.Checkbutton(chart_window, text="显示网格", variable=show_grid_var)
    show_grid_check.grid(row=6, column=0, columnspan=2, pady=5)

    show_labels_var = tk.BooleanVar(value=False)  # 默认不显示数据标签
    show_labels_check = tk.Checkbutton(chart_window, text="显示数据标签", variable=show_labels_var)
    show_labels_check.grid(row=7, column=0, columnspan=2, pady=5)

    tk.Label(chart_window, text="折线样式：").grid(row=8, column=0, padx=10, pady=10)
    line_style_var = tk.StringVar(value="实线")
    line_style_menu = ttk.Combobox(chart_window, textvariable=line_style_var, values=["实线", "虚线", "点线"])
    line_style_menu.grid(row=8, column=1, padx=10, pady=10)

    tk.Label(chart_window, text="颜色主题：").grid(row=9, column=0, padx=10, pady=10)
    color_theme_var = tk.StringVar(value="默认")
    color_theme_menu = ttk.Combobox(chart_window, textvariable=color_theme_var, values=["默认", "深色", "彩色"])
    color_theme_menu.grid(row=9, column=1, padx=10, pady=10)
    ...
    def plot_chart():
        ...
        color_theme = color_theme_var.get()
        ...
        try:
            ...
            if chart_type == "柱状图":
                ...
                if show_labels:
                    for index, value in enumerate(df_copy[y_axis]):
                        ax.text(index, value, str(value), ha="center", va="bottom")

            elif chart_type == "折线图":
                linestyle_dict = {"实线": "-", "虚线": "--", "点线": ":"}
                ax.plot(df_copy[y_axis].values, marker="o", linestyle=linestyle_dict[line_style])
                ax.set_xticks(range(len(df_copy)))
                ax.set_xticklabels(df_copy[x_axis].astype(str).values, rotation=45)
                if show_labels:
                    for i, txt in enumerate(df_copy[y_axis]):
                        ax.text(i, txt, str(txt), ha="center", va="bottom")
            ...
            chart_canvas.draw()
            chart_canvas.get_tk_widget().grid(row=10, column=0, columnspan=2, padx=10, pady=10)
